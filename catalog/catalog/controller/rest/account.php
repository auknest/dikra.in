    ...
    public function notifications() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            if (!$this->customer->isLogged()) {
                ...
                $res = [];
                $this->load->model('account/notifications');
                $customer_id = $this->customer->getId();
                $this->load->model('tool/image');
                $notifications = $this->model_account_notifications->get_notifications(['customer_id' => $customer_id]);
                if ($notifications) {
                    foreach ($notifications as $notification) {

                        if (isset($notification['image']) && is_file(DIR_IMAGE . $notification['image'])) {
                            $image = $this->model_tool_image->resize($notification['image'], 400, 400);
                        } else {
                            $image = '';
                        }
                        $res[] = [
                            ...
                            'description' => html_entity_decode($notification['description']),
                            'image' => $image,
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
            ...
            if (!$this->customer->isLogged()) {
                ...
                if (!isset($post['fcm_token']) || empty($post['fcm_token'])) {
                    $validate = FALSE;
                    $this->error = 'Please provide valid Token';
                }
                if ($validate) {
                    $customer_id = $this->customer->getId();
                    $this->db->query("UPDATE " . DB_PREFIX . "customer SET fcm_token = '" . $this->db->escape($post['fcm_token']) . "' WHERE customer_id = '" . (int) $customer_id . "'");
                } else {
                    $this->json['error'][] = $this->error;
                    $this->statusCode = 400;
                }
