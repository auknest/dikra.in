    ...
    public function loadCatTree($parent = 0, $level = 1) {
        ...
        if ($categories && $level > 0) {
            ...
            foreach ($categories as $category) {
                ...
                if (isset($category['app_image']) && !empty($category['app_image']) && file_exists(DIR_IMAGE . $category['app_image'])) {
                    $image = $this->model_tool_image->resize($category['app_image'], $this->config->get('config_rest_api_image_width'), $this->config->get('config_rest_api_image_height'));
                    $original_image = $this->urlPrefix . 'image/' . $category['app_image'];
                } else {
                    $image = $this->model_tool_image->resize('no_image.png', $this->config->get('config_rest_api_image_width'), $this->config->get('config_rest_api_image_height'));
                    $original_image = $this->urlPrefix . 'image/no_image.png';
                }
...
//            );
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($this->request->post['order_id'])) {
                ...
                if (!in_array($order_info['order_status_id'], $processing_status)) {
                    $this->json['error'][] = "You Can't Cancel Order ... Pleasse contact Support";
                    $this->statusCode = 404;
                } else {
                ...
                $this->json['error'][] = "Please Provide valide Paramters";
                $this->statusCode = 404;
            }
        } else {
            $this->statusCode = 405;
    ...
    private function getOrderDetailsToOrder($order_info) {
        ...
        $orderData['cancel_allow'] = FALSE;
        $orderData['return_allow'] = FALSE;
        if (in_array($order_info['order_status_id'], $processing_status)) {
            $orderData['cancel_allow'] = TRUE;
            $orderData['return_allow'] = FALSE;
        }
        if (in_array($order_info['order_status_id'], $complete_status)) {
            $orderData['cancel_allow'] = FALSE;
            $orderData['return_allow'] = TRUE;
        }
        // Totals
                    ...
                    foreach ($all as $product_info) {
                        ...
                        if ($product_info) {
                            ...
                            $item = array(
                                ...
                                'special_formated' => empty($hidePrices) ? $special_formated : "0",
                                'rating' => $rating,
...
//            );
    ...
    public function slider() {
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "custom_banner  ORDER BY sort_order ASC");
            if (count($query)) {
                foreach ($query->rows as $row) {
                    $slids[] = ['status' => 1, 'slider' => HTTPS_SERVER . 'image/' . $row['image'], 'link' => $row['link']];
                }
                $output = array(
                    'id' => 0,
//                        'settings' => unserialize($row['settings']),
    ...
    public function extra_links() {
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information i INNER JOIN " . DB_PREFIX . "information_description info ON i.information_id=info.information_id");
         $infos=$query->rows;
