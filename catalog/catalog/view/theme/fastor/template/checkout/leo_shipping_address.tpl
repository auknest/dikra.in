<div class="panel panel-default">
    <div class="panel-heading"><i class="fa fa-credit-card" aria-hidden="true"></i> Your Delivery Details</div>
</div>
<?php $adds_count=count($addresses); ?>
<form class="form-horizontal">
    <?php if($adds_count > 0){ ?>
    <div class="form-group">
        <input type="checkbox" id="shipping_address_check"  value="1" checked="checked" class="checkbox1">
        <label>My delivery and billing addresses are the same.</label>
    </div>
    <br/>
    <?php } ?>
    <?php if ($addresses) { ?>
    <div class="radio hidden shipping_check" >
        <label>
    ...
    <?php } ?>
    <br />
    <div id="shipping-new" class="hidden" style="display: <?php echo (($adds_count < 1 && $addresses) ? 'none' : 'block' ); ?>;"> 
        <!--    <div id="shipping-new" class="hidden"> -->
    ...
    <div class="buttons clearfix">
        <div class="pull-right">
            <input type="button" value="<?php echo $button_continue; ?>" id="button-shipping-address" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary hidden" />
        </div>
    </div>
</form>
...
//--></script>
<script type="text/javascript"><!--
$('#delivery_address .form-group[data-sort]').detach().each(function () {
        if ($(this).attr('data-sort') >= 0 && $(this).attr('data-sort') <= $('#delivery_address .form-group').length - 2) {
            $('#delivery_address .form-group').eq(parseInt($(this).attr('data-sort')) + 2).before(this);
        }

        if ($(this).attr('data-sort') > $('#delivery_address .form-group').length - 2) {
            $('#delivery_address .form-group:last').after(this);
        }
        ...
        if ($(this).attr('data-sort') < -$('#delivery_address .form-group').length - 2) {
            $('#delivery_address .form-group:first').before(this);
        }
    });
//--></script>
<script type="text/javascript"><!--
$('#delivery_address button[id^=\'button-shipping-custom-field\']').on('click', function () {
        var node = this;
...
//--></script>
<script type="text/javascript"><!--
    $('#delivery_address select[name=\'country_id\']').on('change', function () {
    ...
    $('#delivery_address select[name=\'country_id\']').on('change', function () {
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function () {
                $('#delivery_address select[name=\'country_id\']').after(' <i class="fa fa-circle-o-notch fa-spin"></i>');
            },
            complete: function () {
                $('.fa-spin').remove();
            },
            success: function (json) {
                if (json['postcode_required'] == '1') {
                    $('#delivery_address input[name=\'postcode\']').parent().parent().addClass('required');
                } else {
                    $('#delivery_address input[name=\'postcode\']').parent().parent().removeClass('required');
                }
                ...
                $('#delivery_address select[name=\'zone_id\']').html(html);
            },
            error: function (xhr, ajaxOptions, thrownError) {
                alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
            }
    ...
    $('#delivery_address select[name=\'country_id\']').trigger('change');
//--></script>
<script type="text/javascript"><!--
/***** Changes Starts *****/
    $('#shipping_address_check').on('click', function () {

        var val = $('#shipping_address_check:checked').val();
        ...
        if (val) {
            $('#shipping-existing').addClass('hidden');
            $('#shipping-new').addClass('hidden');
            $('.shipping_check').addClass('hidden');
        } else {
            $('#shipping-existing').removeClass('hidden');
            $('#shipping-new').removeClass('hidden');
            $('.shipping_check').removeClass('hidden');
        }
    ...
    $('#delivery_address select[name=\'address_id\']').trigger('change');
    payment_method();
    /***** Changes Ends *****/
//--></script>